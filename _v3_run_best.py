import sys as _sys
_sys.path.insert(0, r"D:\Codes\VLSI\PyPlacer_v3_freeze")
import os as _os
_TS   = float(_os.environ.get("TIME_SCALE", "1.0"))
_SEED = int(_os.environ.get("SEED", "42"))
_OUT  = _os.environ.get("OUT_DIR", r"E:\Workbuddy AI\Paper Use\_runs_best")
...
def run_placement(benchmark_dir, output_dir, device='cuda', time_limit=None,
    ...
    # [best-so-far] DP3 可能劣化结果；若属实则回退到 DP3 之前
    _hp_dp3 = bm.compute_hpwl_fast(final_pos_x, final_pos_y,
                                  DPFPGA_X_WEIGHT, DPFPGA_Y_WEIGHT)
    _hp_sw3 = bm.compute_hpwl_fast(round3_x, round3_y,
                                  DPFPGA_X_WEIGHT, DPFPGA_Y_WEIGHT)
    if _hp_sw3 < _hp_dp3:
        print(f"  [best-so-far] DP3 degraded ({_hp_dp3:.0f} -> {_hp_sw3:.0f}), "
              f"reverting to pre-DP3")
        final_pos_x, final_pos_y = round3_x, round3_y
    else:
        print(f"  [best-so-far] DP3 improved or equal ({_hp_sw3:.0f} -> {_hp_dp3:.0f})")
