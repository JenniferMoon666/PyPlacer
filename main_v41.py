"""PyPlacer V41: PyTorch scatter_reduce Batch DP
...
def run_placement(benchmark_dir, output_dir, device='cuda', time_limit=None,
    ...
    print(f"  PyPlacer V41 (PyTorch scatter_reduce Batch DP): {mode}")
    ...
    # For small circuits, DPFPGA LG often produces worse results than our own LG
    # So we skip DPFPGA for small circuits even with --dpfpga flag
    use_dpfpga_actual = use_dpfpga and not is_small
    if use_dpfpga and is_small:
        print(f"  [INFO] Small circuit ({num_movable} movable), using PyPlacer GP+LG instead of DPFPGA")

    if use_dpfpga_actual or load_pl_file:
        if load_pl_file:
